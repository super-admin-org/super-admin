@include("admin::form._header")
        ...
        <div class="flex flex-wrap gap-2 radio-group-toggle">
            @foreach($options as $option => $label)
                <label class="cursor-pointer">
                    <input type="radio" name="{{$name}}" value="{{$option}}" class="sr-only peer minimal {{$class}}" {{ ($option == old($column, $value)) || ($value === null && in_array($label, $checked)) ?'checked':'' }} {!! $attributes !!} />
