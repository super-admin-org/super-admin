        ...
        <div class="flex flex-wrap gap-2">
            @foreach($options as $option => $label)
                <label class="cursor-pointer">
                    <input type="radio" name="{{$name}}" value="{{$option}}" id="{{$name}}-{{$option}}" class="sr-only peer {{$class}}" {{ ($option == old($column, $value)) || ($value === null && in_array($label, $checked)) ?'checked':'' }} {!! $attributes !!} />
