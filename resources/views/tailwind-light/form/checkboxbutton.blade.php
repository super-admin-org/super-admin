@include("admin::form._header")
        ...
        <div class="flex flex-wrap gap-2 checkbox-group-toggle">
        @foreach($options as $option => $label)
            <label class="cursor-pointer">
                <input type="checkbox" name="{{$name}}[]" value="{{$option}}" class="sr-only peer {{$class}}" {{ false !== array_search($option, array_filter(old($column, $value ?? []))) || ($value === null && in_array($option, $checked)) ?'checked':'' }} {!! $attributes !!} />
