<div class="flex">
    ...
    <div class="relative">
        ...
        <button type="button" class="glass-btn-secondary text-sm rounded-r-none whitespace-nowrap" data-sa-toggle="dropdown" data-sa-target="#filter-group-{{ $id }}">
    ...
    <div class="relative flex items-center @if($group) flex-1 @endif">
        <span class="absolute left-3 text-gray-400"><i class="icon-{{ $icon }}"></i></span>
        <input type="{{ $type }}" class="glass-input pl-8 @if($group) rounded-l-none @endif {{ $id }}" placeholder="{{$placeholder}}" name="{{$name}}" value="{{ request($name, $value) }}">
    </div>
</div>
