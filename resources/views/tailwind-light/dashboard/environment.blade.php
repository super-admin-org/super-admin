<div class="glass-card overflow-hidden">
    <div class="flex items-center justify-between px-5 py-3 border-b border-gray-200/30">
        <h3 class="text-sm font-semibold text-gray-700">Environment</h3>
        <button data-sa-toggle="collapse" data-sa-target="#env-body" class="p-1 rounded hover:bg-gray-100/50 text-gray-400 transition-colors">
        ...
        <div class="overflow-x-auto">
            <table class="glass-table">
                <tbody>
                    @foreach($envs as $env)
                    <tr>
                        <td class="font-medium text-gray-600 w-36">{{ $env['name'] }}</td>
                        <td>{{ $env['value'] }}</td>
                    </tr>
                    @endforeach
                </tbody>
